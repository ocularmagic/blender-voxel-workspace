...
| **D6** | **One material**; mesh carries CORNER-domain `INT` `palette_index`; shader samples nearest from a 256×1 palette image at `(index + 0.5) / 256`. Indices 1–7 rendered in Blender 5.1 EEVEE (`BLENDER_EEVEE`) and Cycles with one slot (PNG RMSE 0.0472). Do not rely on setting deprecated `Material.use_nodes` in 5.1+. | **validated (003)** | 003 PARTIAL verdict |
